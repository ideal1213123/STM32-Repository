...
#define LCD_ClearRect(mode,xp,yp,w,h,clr) do{\
	if(mode){\
		unsigned short nx  = yp, ny = xp;\
		unsigned short nx2 = nx+h-1, ny2 = ny+w-1;\
		nx2 = nx2>=LCD_W?LCD_W-1:nx2;\
		ny2 = ny2>=LCD_H?LCD_H-1:ny2;\
		for(unsigned short j=nx; j<=nx2; ++j ){\
			LCD_SetCursor(j,LCD_H-ny-1);\
			LCD_WriteRAM_Prepare();\
			for(unsigned short i=ny; i<=ny2; ++i ){ LCD_WriteRAM(clr); }\
		}\
	}else{\
		unsigned short nx = xp, ny = yp;\
		unsigned short nx2 = nx+w-1, ny2 = ny+h-1;\
		nx2 = nx2>=LCD_W?LCD_W-1:nx2;\
		ny2 = ny2>=LCD_H?LCD_H-1:ny2;\
		for(unsigned short j=ny; j<=ny2; ++j ){\
			LCD_SetCursor(xp,j);\
			LCD_WriteRAM_Prepare();\
			for(unsigned short i=nx; i<=nx2; ++i ){ LCD_WriteRAM(clr); }\
		}\
	}\
}while(0)
...
#define LCD_DrawChar(mode,xp,yp,ch,clrf,clrb) do{\
	...
	for(unsigned char j=0; j<24; ++j ){\
		if(mode){ LCD_SetCursor(nx+j,LCD_H-ny-1); }\
